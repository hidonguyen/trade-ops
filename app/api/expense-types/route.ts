// Expense Types list + create — all roles can read, only ADMIN can write
...
import { expenseTypesKey, TAG, TTL } from "@/lib/cache/keys";
...
export async function GET(request: Request) {
  ...
  try {
    const data = await withCache(
      {
        key: `${expenseTypesKey()}:all=${includeInactive}`,
        tags: [TAG.expenseTypes],
        ttlMs: TTL.catalog,
      },
    ...
    console.error("GET /api/expense-types error:", error);
...
export async function POST(request: Request) {
  ...
  const validation = createExpenseTypeSchema.safeParse(body);
  ...
  try {
    const result = await prisma.$transaction(async (tx: any) => {
      const created = await tx.expenseType.create({ data: validation.data });
    ...
    console.error("POST /api/expense-types error:", error);
    return Response.json(apiResponse(false, undefined, MSG.internalError), { status: 500 });
  }
}
