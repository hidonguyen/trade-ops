// Purchase detail Excel export — GET /api/reports/purchase-detail/export
// Returns 14-column per-payment detail workbook (.xlsx) for PURCHASE orders in a date range
...
export async function GET(request: Request) {
  ...
  if (!checkAccess(session.user.roles, "GET", "PURCHASE")) {
  ...
  try {
    ...
    const buffer = await exportPurchaseDetail(mapped, fromDate, toDate);
    const filename = buildReportFilename("mua-hang-chi-tiet", fromDate, toDate);
    ...
    console.error("GET /api/reports/purchase-detail/export error:", error);
